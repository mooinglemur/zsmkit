...
extern void __fastcall__ zsm_setlfs(uint8_t lfnsa, uint8_t priority, uint8_t
device); extern void __fastcall__ zsm_setfile(uint8_t priority, char *filename);
extern uint16_t __fastcall__ zsm_loadpcm(uint8_t priority, uint16_t addr,
attenuation); extern void __fastcall__ zsm_setcb(uint8_t priority, zsm_callback,
uint8_t bank); extern void __fastcall__ zsm_clearcb(uint8_t priority); extern
struct zsm_state __fastcall__ zsm_getstate(uint8_t priority); extern void
__fastcall__ zsm_setrate(uint8_t priority, uint16_t tickrate); extern uint16_t
__fastcall__ zsm_getrate(uint8_t priority); extern void __fastcall__
zsm_setloop(uint8_t priority, uint8_t loop); extern void __fastcall__
zsm_opmatten(uint8_t priority, uint8_t channel, uint8_t val); extern void
__fastcall__ zsm_psgatten(uint8_t priority, uint8_t channel, uint8_t val);
extern void __fastcall__ zsm_pcmatten(uint8_t priority, uint8_t channel, uint8_t
val); extern void __fastcall__ zsm_set_int_rate(uint8_t herz, uint8_t in256th);

bank); extern void __fastcall__ zcm_play(uint8_t slot, uint8_t volume); extern
void __fastcall__ zcm_stop(); extern void __fastcall__ zsmkit_setisr(); extern
void __fastcall__ zsmkit_clearisr();
*/
